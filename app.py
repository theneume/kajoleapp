...
def send_hi():
    ...
    for match in user_matches:
        if match.get('candidate_id') == candidate_id and match.get('match_date') == today:
            ...
            # Get user info — use embedded candidate for demo profiles
            user = get_user(user_id) or {}
            candidate = match.get('candidate') or get_user(candidate_id) or {}

            # Standardised conv_id: always userId_candidateId
            conv_id = f"{user_id}_{candidate_id}"

            # Send message using dict format (db_layer expects dict)
            send_message({
            ...
            return jsonify({
                "success": True,
                "conversation_id": conv_id,
                "message": f"Your Hi has been sent to {candidate.get('name', 'your match')}. The conversation begins."
            })

    return jsonify({"error": "Match not found for today"}), 404


@app.route('/api/match/open_conversation', methods=['POST'])
def open_conversation():
    """Open or retrieve a conversation with any matched profile (including demo profiles)."""
    user_id = get_current_user_id()
    if not user_id:
        return jsonify({"error": "Not authenticated"}), 401
    ...
    # Find the match record to get embedded candidate data
    user_matches = get_user_matches(user_id)
    match_record = None
    ...
    if not match_record:
        return jsonify({"error": "No match record found for this candidate"}), 404

    # Update status to conversation if not already
    if match_record.get('status') == 'pending':
        update_match(match_record.get('id'), {'status': 'hi_sent'})

    candidate = match_record.get('candidate') or get_user(candidate_id) or {}
    conv_id = f"{user_id}_{candidate_id}"
...
def ai_chat():
    """
    AI Match Architect chat — stateless, fetches all data fresh inside the function.
    No global variable passing. Every call fetches user + match context from DB.
    Wrapped entirely in try/except — never returns a 500.
    """
    try:
        # ── 1. Auth ──────────────────────────────────────────────────────────
        user_id = get_current_user_id()
        ...
        # ── 2. Fresh data fetch (stateless — no globals) ─────────────────────
        user = get_user(user_id)
        if not user:
            return jsonify({"error": "User not found"}), 404

        # Fetch today's match for context — use embedded candidate (works for demo profiles)
        user_matches = get_user_matches(user_id)
        today_match = None
        for m in (user_matches or []):
            if m.get('match_date') == date.today().isoformat():
                today_match = m
                break
        # Fall back to last match if no today match
        if not today_match and user_matches:
            today_match = user_matches[-1]

        match_context = None
        if today_match:
            candidate = today_match.get('candidate') or get_user(today_match.get('candidate_id', '')) or {}
            match_context = {
                "candidate": candidate,
                "compatibility": today_match.get('compatibility', {})
            }

        # ── 3. Silently extract preference signals (non-blocking) ────────────
        if AI_ENGINE_AVAILABLE:
            try:
                ...
                print(f"match spec extraction error: {spec_err}", file=sys.stderr)

        # ── 4. Fetch conversation history fresh ──────────────────────────────
        history = user.get('ai_conversation', []) or []
        ...
        try:
            ...
            print(f"AI call error: {type(ai_err).__name__}: {ai_err}", file=sys.stderr, flush=True)

        # ── 6. Fallback if AI failed or returned None ─────────────────────────
        if not response:
            print("KAJOLE AI: using utility fallback", flush=True)
            response = (
                "I'm having trouble pulling your latest match data. "
                "Let me refresh the connection — try sending that again in a moment."
            )

        # ── 7. Save conversation (keep last 20 messages) ──────────────────────
        ...
        # Absolute last-resort catch — never let this endpoint 500
        print(f"AI chat outer error: {type(outer_err).__name__}: {outer_err}", file=sys.stderr, flush=True)
        return jsonify({
            "response": (
                "I'm having trouble pulling your latest match data. "
                "Let me refresh the connection — try sending that again in a moment."
...
def inbox():
    ...
    for match in user_matches:
        ...
        if match.get('status') in ['hi_sent', 'conversation', 'active']:
            cand_id = match.get('candidate_id')
            # CRITICAL: use embedded candidate first (works for demo profiles)
            candidate = match.get('candidate') or get_user(cand_id) or {}
            # Standardised conv_id: always userId_candidateId (no conv_ prefix)
            conv_id = f"{user_id}_{cand_id}"
            messages = get_conversation_messages(conv_id)  # from db_layer
