  ...
  try {
    ...
    if (!resp.ok) {
      ...
      try {
        ...
        return { error: errorData.error || `Server error: ${resp.status}` };
      } catch {
...
function renderMatchCard(candidate, matchRecord, alreadySentHi) {
  ...
  // Message button always visible — works before and after Hi is sent
  const messageButton = `<button class="btn btn-primary btn-message" onclick="openDirectMessage('${candidate.id}')">
...
async function openDirectMessage(candidateId) {
  ...
  // Get candidate from current match state or history
  let candidate = null;
  if (STATE.todayMatch?.candidate?.id === candidateId) {
    candidate = STATE.todayMatch.candidate;
  }

  // Call open_conversation to ensure match status is updated and get conv_id
  try {
    const result = await apiCall('/match/open_conversation', 'POST', { candidate_id: candidateId });
    if (result.error) {
      // Fallback: just build conv_id and try anyway
      const convId = `${userId}_${candidateId}`;
      switchView('inbox');
      setTimeout(async () => {
        await loadInbox();
        if (candidate) setTimeout(() => openConversation(convId, candidate), 350);
      }, 250);
      return;
    }
    const convId = result.conversation_id || `${userId}_${candidateId}`;
    ...
    setTimeout(async () => {
      ...
      if (resolvedCandidate) {
        setTimeout(() => openConversation(convId, resolvedCandidate), 350);
      }
    }, 250);
  } catch (err) {
    // Fallback
